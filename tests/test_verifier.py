...
def test_verify_document_by_doi():
    ...
    verifier = DocumentVerifier(crossref_provider=mock_crossref, openalex_provider=MagicMock())
    doc = Document(title="Attention Is All You Need", external_ids=ExternalIds(doi="10.5555/3295222.3295349"))
...
def test_verify_document_hallucination_detection():
    ...
    verifier = DocumentVerifier(crossref_provider=mock_crossref, openalex_provider=mock_openalex)
    fake_doc = Document(title="A Fake Hallucinated Paper On Quantum Banana Peels", external_ids=ExternalIds(doi="10.1234/fake.doi"))
...
def test_hydrate_metadata():
    mock_openalex = MagicMock()
    mock_openalex.base_url = "https://api.openalex.org/works"
    mock_openalex.client.get.return_value.json.return_value = {"id": "W123", "title": "Real Paper"}
    ...
    incomplete_doc = Document(title="Real Paper", external_ids=ExternalIds(doi="10.1038/real"))
