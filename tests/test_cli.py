...
def test_cli_snowball(tmp_path: Path):
    ...
    with patch("scholar_search.cli.SearchEngine.snowball_forward", return_value=[dummy_doc]):
        result = runner.invoke(app, [
            "snowball", "W2741809807",
            "--provider", "openalex",
            "--direction", "forward",
            "--output", str(output_file)
        ])
...
def test_cli_import_and_dedup(tmp_path: Path):
    ...
    input_file.write_text('[{"title": "Paper A", "doi": "10.1000/1"}, {"title": "Paper A", "doi": "10.1000/1"}]')
    output_file = tmp_path / "deduped.json"

    # Test import command
    result_import = runner.invoke(app, ["import", str(input_file), "--output", str(output_file)])
    assert result_import.exit_code == 0
    assert "Loaded 2 records" in result_import.stdout

    # Test dedup command
    result_dedup = runner.invoke(app, ["dedup", str(input_file), "--output", str(output_file)])
