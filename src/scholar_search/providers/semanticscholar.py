...
from ..models import Document, Query, Author, ExternalIds
...
class SemanticScholarProvider(BaseAPIProvider):
    ...
    def __init__(self) -> None:
        ...
        self.fields = "paperId,externalIds,title,abstract,year,authors,venue,url,referenceCount,citationCount,tldr"
    ...
    def _normalize_document(self, raw: Dict[str, Any], query_id: str | None = None, citation_intent: list[str] | None = None) -> Document:
    ...
    def search(self, query: Query) -> Iterator[Document]:
        ...
        params = {
            ...
            "fields": "paperId,title,abstract,year,authors,venue,externalIds,url,citationCount,referenceCount,tldr",
        }
        ...
        while True:
            ...
            response_data = self.client.get(self.base_url, params=params).json()
    ...
    def get_citations(self, document_id: str) -> Iterator[Document]:
        ...
        params = {
            "fields": self.fields + ",contexts,intents",
            "limit": 1000
        }
        response = self.client.get(f"{self.base_url.replace('/search/bulk', '')}/{document_id}/citations", params=params, headers=self._get_headers()).json()
    ...
    def get_references(self, document_id: str) -> Iterator[Document]:
        ...
        response = self.client.get(f"{self.base_url.replace('/search/bulk', '')}/{document_id}/references", params=params, headers=self._get_headers()).json()
