...
class OpenAlexProvider(BaseAPIProvider):
    ...
    def _normalize_document(self, raw: Dict[str, Any], query_id: str | None = None) -> Document:
        ...
        for authorship in raw.get("authorships", []):
            ...
            if len(name_parts) > 1:
                ...
                authors.append(Author(family_name=name_parts[0], orcid=author_data.get("orcid")))
        ...
        doc = Document(
            ...
            abstract=self._parse_abstract_inverted_index(raw.get("abstract_inverted_index")),
            authors=authors,
            venue=venue_name,
    ...
    def _parse_abstract_inverted_index(self, inverted_index: Dict[str, list[int]] | None) -> str | None:
    ...
    def search(self, query: Query) -> Iterator[Document]:
        ...
        count = 0
        while True:
            response = self.client.get(self.base_url, params=params).json()
    ...
    def get_references(self, document_id: str) -> Iterator[Document]:
        ...
        doc_response = self.client.get(f"{self.base_url}/{document_id}").json()
