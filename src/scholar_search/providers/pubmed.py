...
class PubMedProvider(BaseAPIProvider):
    ...
    def _fetch_details(self, pmids: list[str], query_id: str | None = None) -> Iterator[Document]:
        ...
        for article_elem in root.findall('.//PubmedArticle'):
            ...
            for author_elem in article.findall('.//Author'):
                ...
                first = author_elem.findtext('ForeName') or author_elem.findtext('Initials')
                if last:
    ...
    def search(self, query: Query) -> Iterator[Document]:
        ...
        search_response = self.client.get(f"{self.base_url}/esearch.fcgi", params=params).json()
        ...
        for i in range(0, len(pmids), chunk_size):
            ...
            for doc in self._fetch_details(chunk, query.id):
                yield doc
                count += 1
    ...
    def get_citations(self, document_id: str) -> Iterator[Document]:
        ...
        response = self.client.get(f"{self.base_url}/elink.fcgi", params=params).json()
        ...
        yield from self._fetch_details(citing_pmids)
