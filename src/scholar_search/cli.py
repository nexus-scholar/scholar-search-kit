...
def _get_provider_instance(name: str):
    ...
    if name_clean == "openalex":
        ...
        raise typer.BadParameter(f"Unknown provider '{name}'. Options: openalex, crossref, pubmed, arxiv, semanticscholar, biorxiv")
...
def _save_output(documents: List[Document], output_path: Path, format_str: str) -> None:
    ...
    if format_clean == "json":
        ...
        raise typer.BadParameter(f"Unsupported format '{format_str}'. Options: json, jsonl, csv")
    console.print(f"[green]Saved {len(documents)} documents to {output_path} ({format_clean.upper()})[/green]")
...
def _display_documents_table(documents: List[Document], title: str = "Search Results") -> None:
    ...
    for doc in documents[:25]:
        doi_or_id = doc.external_ids.doi or doc.external_ids.arxiv_id or doc.external_ids.pubmed_id or doc.provider_id or "N/A"
...
@app.command()
def search(
    query_text: str = typer.Argument(..., help="Search query (e.g. 'machine learning AND transformer')"),
    provider: Optional[str] = typer.Option(None, "--provider", "-p", help="Specific provider (openalex, crossref, pubmed, arxiv, semanticscholar, biorxiv) or all"),
    limit: int = typer.Option(20, "--limit", "-l", help="Maximum documents to retrieve"),
    year_min: Optional[int] = typer.Option(None, "--year-min", help="Earliest publication year"),
    year_max: Optional[int] = typer.Option(None, "--year-max", help="Latest publication year"),
    dedup: bool = typer.Option(True, "--dedup/--no-dedup", help="Automatically cluster and deduplicate results"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Filepath to save results"),
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
        progress.add_task(description=f"Querying academic providers for '{query_text}'...", total=None)
        results = engine.search_all(q, dedup=dedup)
...
@app.command()
def snowball(
    doc_id: str = typer.Argument(..., help="Target document ID (e.g. OpenAlex ID like 'W2741809807' or Semantic Scholar Paper ID)"),
    provider: str = typer.Option("openalex", "--provider", "-p", help="Provider for snowballing (openalex, semanticscholar, pubmed)"),
    direction: str = typer.Option("forward", "--direction", "-d", help="Snowball direction: forward (citing papers) or backward (references)"),
    limit: int = typer.Option(25, "--limit", "-l", help="Maximum citations/references to retrieve"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Filepath to save results"),
    format: str = typer.Option("json", "--format", "-f", help="Output format: json, jsonl, csv")
):
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
        if direction.lower() == "forward":
            progress.add_task(description=f"Finding papers citing {doc_id} on {provider}...", total=None)
            results = engine.snowball_forward(doc_id, provider)
        elif direction.lower() == "backward":
            progress.add_task(description=f"Finding references cited by {doc_id} on {provider}...", total=None)
            results = engine.snowball_backward(doc_id, provider)
        else:
            raise typer.BadParameter("Direction must be 'forward' or 'backward'")

    limited_results = results[:limit]
    _display_documents_table(limited_results, title=f"Snowball ({direction.capitalize()}) Results for: {doc_id}")
...
def import_citations(
    file_path: Path = typer.Argument(..., help="Path to .ris, .json, or .jsonl citation file", exists=True),
    verify: bool = typer.Option(False, "--verify", "-v", help="Verify existence against Crossref/OpenAlex and detect hallucinations"),
    enrich: bool = typer.Option(False, "--enrich", "-e", help="Hydrate missing abstracts, venues, and citations from OpenAlex"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Filepath to save normalized results"),
    format: str = typer.Option("json", "--format", "-f", help="Output format: json, jsonl, csv")
):
    ...
    if suffix == ".ris":
        ...
        raise typer.BadParameter(f"Unsupported file type '{suffix}'. Use .ris, .json, or .jsonl")
    ...
    if verify or enrich:
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
            progress.add_task(description="Verifying and hydrating document metadata...", total=None)
            documents, audit = verifier.process_batch(documents, verify=verify, enrich=enrich)
            
        verified_count = sum(1 for a in audit if a["verified"])
        console.print(f"[green]Verified: {verified_count}/{len(documents)} documents confirmed real.[/green]")
        if verified_count < len(documents):
            console.print(f"[yellow]Warning: {len(documents) - verified_count} records could not be verified in Crossref/OpenAlex.[/yellow]")
...
@app.command()
def dedup(
    input_file: Path = typer.Argument(..., help="Path to input JSON, JSONL, or RIS file", exists=True),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Filepath to save deduplicated documents"),
    format: str = typer.Option("json", "--format", "-f", help="Output format: json, jsonl, csv")
):
    ...
    console.print(f"  * Duplicates merged: [yellow]{stats['duplicates']} ({stats['duplicate_rate']:.1%})[/yellow]")
...
@app.command()
def export(
    input_file: Path = typer.Argument(..., help="Input file path (.json, .jsonl, .ris)", exists=True),
    output_file: Path = typer.Argument(..., help="Output destination file path"),
    format: str = typer.Option("json", "--format", "-f", help="Target format: json, jsonl, csv")
):
